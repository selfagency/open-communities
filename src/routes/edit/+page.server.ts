		...
		if (client?.id) {
			...
			const congregation = cleanResponse(
				await api
					...
					.getFirstListItem(api.filter("id={:id}", { id }), { fetch }),
			) as RecordWithId;
		...
		try {
			...
			if (accessibility.id) {
				batch
					...
					.update(accessibility.id, omit(accessibility, ["id"]));
			} else if (!isEmpty(accessibility)) {
				batch
					.collection("accessibility")
					.create({ ...accessibility, congregation: data.id });
			}
			if (fit.id) {
			...
			if (registration.id) {
				batch
					...
					.update(registration.id, omit(registration, ["id"]));
			} else if (!isEmpty(registration)) {
				batch
					.collection("registration")
					.create({ ...registration, congregation: data.id });
			}
			if (health.id) {
			...
			if (security.id) {
				batch
					.collection("security")
					.update(security.id, omit(security, ["id"]));
			} else if (!isEmpty(security)) {
				batch
					.collection("security")
					.create({ ...security, congregation: data.id });
			}
			if (services.id) {
				batch
					.collection("services")
					.update(services.id, omit(services, ["id"]));
			} else if (!isEmpty(services)) {
				batch
					.collection("services")
					.create({ ...services, congregation: data.id });
			}
			await batch.send({ fetch });
			...
			if (!client?.admin) {
				...
				await transactionalMail({
					email: c.email,
					message: `${m.transactional_updated({ locale: c.lang || "en" })} ${m[
				...
				data.owner &&
				form.data.visible &&
				!priorToChange.visible
			) {
				const owner = await api
					.collection("users")
					.getOne(data.owner, { fetch });
				await transactionalMail({
					email: owner.email,
	...
	transfer: async (event) => {
		...
		try {
			...
			const user = await api
				.collection("users")
				.getFirstListItem(api.filter("email={:email}", { email: data.email }), {
					fetch,
				});
