...
export const userHandlers = [
	...
	http.post(`${PB}/api/collections/users/auth-refresh`, async ({ request }) => {
		...
		if (!user) {
			return HttpResponse.json(
				{ code: 401, message: "Invalid token.", data: {} },
				{ status: 401 },
			);
		}
		return HttpResponse.json({
	...
	http.post(
		...
		async ({ request }) => {
			...
			if (!body.token) {
				return HttpResponse.json(
					{ code: 400, message: "Invalid token.", data: {} },
					{ status: 400 },
				);
			}
			return HttpResponse.json({});
	...
	http.post(
		`${PB}/api/collections/users/confirm-verification`,
		async ({ request }) => {
			const body = (await request.json()) as { token?: string };
			if (!body.token) {
				return HttpResponse.json(
	...
	http.patch(
		`${PB}/api/collections/users/records/:id`,
		async ({ params, request }) => {
			const body = (await request.json()) as Record<string, unknown>;
			const user = findUserById(params.id as string);
			if (!user) {
				return HttpResponse.json(
					{ code: 404, message: "User not found.", data: {} },
					{ status: 404 },
				);
			}
