...
import {
	allCongregations,
	congregationMetaViews,
	findCongregationById,
} from "../../data/congregations";
...
export const congregationHandlers = [
	...
	http.get(
		`${PB}/api/collections/congregationMeta/records/:id`,
		({ params }) => {
			const cong = findCongregationById(params.id as string);
			if (!cong) {
				return HttpResponse.json(
					{ code: 404, data: {}, message: "The resource wasn't found." },
					{ status: 404 },
				);
			}
	...
	http.post(
		`${PB}/api/collections/congregations/records`,
		async ({ request }) => {
			...
			allCongregations.push(
				newCong as CongregationFixture & Record<string, unknown>,
			);
	...
	http.patch(
		`${PB}/api/collections/congregations/records/:id`,
