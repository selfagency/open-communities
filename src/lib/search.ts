				...
				const locationIds = this.data
					.filter((record) => {
						const {
							...
							country,
							state: recordState,
						} = record.location as LocationMeta;
						const cityMatch =
							!filterCity || (city && city.id === filterCity.id);
						const countryMatch =
							!filterCountry || (country && country.id === filterCountry.id);
						const stateMatch =
							!filterState ||
							(recordState && recordState.id === filterState.id);
	...
	private _buildIndexes(): void {
		this.data.forEach((record, idx) => {
			...
			for (const filter of ["health", "registration"] as const) {
				...
				if (sub) {
					const targetKey =
						filter === "health" ? "protocol" : "registrationType";
