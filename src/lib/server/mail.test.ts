...
afterEach(async () => {
	try {
		...
		try {
			...
			console.debug(
				"Mailpit not reachable for cleanup (fetch failed), skipping DELETE",
			);
			return;
		}
	...
	it("sends admin email with listing appended when record present", async () => {
		...
		while (Date.now() < rawDeadline) {
			const rawRes = await fetch(
				`http://localhost:8025/api/v1/messages/${found!.id}/raw`,
				{
					headers: {
						accept: "application/json",
					},
		...
		if (!raw) {
			const detailRes = await fetch(
				`http://localhost:8025/api/v1/message/${found!.id}`,
				{
					headers: {
						accept: "application/json",
					},
			...
			if (detailRes.ok) {
				...
				try {
					...
					if (foundMessage && foundMessage.Snippet) {
						expect(foundMessage.Snippet).toContain(
							"Listing: Congregation Name",
						);
						return; // Test passed, exit early
					}
				} catch (e) {
					console.debug(
						"Failed to fetch Mailpit messages list for diagnostics:",
						e,
					);
				}
