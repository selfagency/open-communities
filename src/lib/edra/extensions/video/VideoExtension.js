...
export const Video = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['video', { controls: 'true', style: 'width: fit-content;', ...HTMLAttributes }, ['source', HTMLAttributes]];
  },
  addCommands() {
    return {
      setVideo:
        (src) =>
        ({ commands }) =>
          commands.insertContent(`<video controls="true" autoplay="false" style="width: fit-content" src="${src}" />`),
      toggleVideo:
        () =>
        ({ commands }) =>
