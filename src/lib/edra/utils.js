 ...
 */
export function getHandlePaste(editor, maxSize = 2) {
  return (_view, event) => {
    ...
    if (filesize && Number(filesize) > maxSize) {
      globalThis.alert(`too large image! filesize: ${filesize} mb`);
      return;
    }
 ...
 */
export const duplicateContent = (editor, node) => {
  const { view } = editor;
  const { state } = view;
 ...
 */
export function focusEditor(editor, event) {
  if (!editor) return;
