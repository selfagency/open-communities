...
describe('Registration segment (behavior)', () => {
	it('updates registration email and url on input/change', async () => {
		...
		const props = makeMockFormProps(
			{ registration: { email: '', otherText: '', registrationType: '', url: '' } },
			{}
		);
		...
		const emailInput =
			inputs.find((i) => i.getAttribute('id')?.includes('registration_email')) || inputs[0];
		emailInput.value = '  user@example.com  ';
		emailInput.dispatchEvent(new Event('input', { bubbles: true }));
		emailInput.dispatchEvent(new Event('change', { bubbles: true }));

		const urlInput =
			inputs.find((i) => i.getAttribute('id')?.includes('registration_url')) || inputs[1];
		...
		const reg = (latest as unknown as Record<string, unknown>).registration as
