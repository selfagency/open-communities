...
describe('Health segment (behavior)', () => {
  it('selects a protocol radio and updates formData', async () => {
    ...
    const radios = target.querySelectorAll('input[type="radio"], [role="radio"], [data-slot="radio-group-item"]');
    ...
    const health = (latest as unknown as unknown as Record<string, unknown>).health as unknown as Record<string, unknown> | undefined;
