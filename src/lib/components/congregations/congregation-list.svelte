...
const searchStrings = $derived(
  allCongs.map((c) => `${c.name} ${c.denomination ?? ''} ${c.city ?? ''} ${c.state ?? ''}`)
);
...
async function confirmAction() {
  ...
  await fetch('/api/admin/congregations/' + pendingId + (pendingAction === 'approve' ? '/toggle' : '/delete'), {
    method: pendingAction === 'approve' ? 'POST' : 'DELETE'
  });
  pendingId = null;
  pendingAction = null;
...
const nameCell = (v: string) =>
  renderSnippet(
    createRawSnippet<[{ v: string }]>((get) => ({ render: () => `<span class="font-medium">${get().v}</span>` })),
    { v }
  );
const denomCell = (v: string) =>
  renderSnippet(
    createRawSnippet<[{ v: string }]>((get) => ({ render: () => `<span class="capitalize">${get().v || '—'}</span>` })),
    { v }
  );
const mutedCell = (v: string) =>
  renderSnippet(
    createRawSnippet<[{ v: string }]>((get) => ({
      render: () => `<span class="text-muted-foreground text-xs">${get().v || '—'}</span>`
    })),
    { v }
  );
    ...
    <div class="flex items-center gap-2">
      ...
      {#if search}
        <p class="text-muted-foreground text-sm">
          {m.searchResults({ count: filtered.filter(c => c.visible).length })}
        </p>
      {/if}
    </div>
  </div>
