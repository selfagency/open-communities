...
describe('Security component', () => {
	...
	it('full: lists all armed reasons when present', () => {
		...
		expect(
			within(list).getByText((content) => content.includes(m.security_localPolice()))
		).toBeInTheDocument();
		expect(
			within(list).getByText((content) => content.includes(m.security_privateSecurityArmed()))
		).toBeInTheDocument();
		expect(
			within(list).getByText((content) => content.includes(m.security_clergyArmed()))
		).toBeInTheDocument();
		expect(
			within(list).getByText((content) => content.includes(m.security_congregantsArmed()))
		).toBeInTheDocument();
	});
