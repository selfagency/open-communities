...
describe('Registration component', () => {
	...
	it('renders email and website buttons when provided', () => {
		...
		expect(links.some((l) => (l as HTMLAnchorElement).href.includes('mailto:r@test.com'))).toBe(
			true
		);
		expect(links.some((l) => (l as HTMLAnchorElement).href.includes('https://example.com'))).toBe(
			true
		);
